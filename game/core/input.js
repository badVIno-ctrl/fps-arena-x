...
export const ACTIONS = {
  ...
  // is taught in any UI yet, so rather than break existing muscle memory or
...
export class Input {
  constructor(canvas, config) {
    ...
    this.frozen = false;
    /**
     * Set while a full-screen UI owns the mouse (the gunsmith board, any future
     * menu that wants real clicks). A left click normally re-grabs pointer lock,
     * which is right in a match and wrong on a board: the cursor vanished the
     * instant the player tried to press a button, so nothing on the board could
     * be clicked at all. The board still wants mouse BUTTONS (it drags the gun to
     * spin it), so this suppresses only the lock grab, not the event.
     */
  ...
  _onMouseDown(e) {
    if (!this.enabled) return;
    if (!this.pointerLocked && !this.lockSuppressed && e.button === 0) this.requestPointerLock();
