...
/**
 * Catch NaN vertices at the point of entry, while we still know who made them.
 *
 * A single NaN position poisons a whole merged buffer: the bounding sphere goes
 * NaN, the frustum test stops being meaningful, and three.js reports
 * "Computed radius is NaN" from deep inside the render loop — long after the
 * builder that produced the bad number has returned. That error names no part
 * and no weapon, which makes it nearly untraceable in a 400k-triangle arsenal.
 *
 * Checking here costs one pass over the position buffer at build time (never per
 * frame) and turns that anonymous render-loop warning into the assembly and
 * material that caused it. Undefined placement values are the usual culprit:
 * `undefined` in a transform silently becomes NaN in the matrix.
 */
function assertFinite(geo, owner, mat) {
  const pos = geo.getAttribute('position');
  if (!pos) return;
  const a = pos.array;
  for (let i = 0; i < a.length; i++) {
    if (Number.isFinite(a[i])) continue;
    const vert = Math.floor(i / 3);
    throw new Error(
      `[geometry] non-finite vertex in "${owner}" (material "${mat}"): ` +
        `component ${i % 3} of vertex ${vert} is ${a[i]}. ` +
        `A transform value was probably undefined.`,
    );
  }
}
...
export class Assembly {
  ...
  /**
   * Pull everything out of `other` into this assembly, offset by `t`.
   *
   * Some part builders (`buildMagazine`, `buildSlide`) deliberately work in
   * their own space with the origin on a feature — the feed lips, the breech
   * face — because that is the only frame in which their internal maths stays
   * readable. Their callers then need that whole body positioned somewhere
   * else, and re-deriving every internal offset at the call site is how parts
   * silently drift apart.
   *
   * `other` is emptied, so it can be a throwaway scratch assembly.
   */
  absorb(other, t = null) {
    for (const [mat, list] of other.buckets) {
      for (const g of list) {
        this.add(g, mat, t);
        g.dispose();
      }
