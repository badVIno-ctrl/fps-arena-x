...
/**
 * Wall press (упор в стену).
 *
 * Rounds spawn at the muzzle but fly along camera-forward, so pressing a long
 * barrel into geometry would otherwise spawn bullets on the far side of the wall
 * and leave the gun visibly buried in it. The weapon is graded into a low-ready
 * as a wall closes in, using the SAME blend mantling already drives.
 *
 * Distances are measured from the eye along the aim line and are relative to the
 * live muzzle reach, so a pistol can be taken far closer to a wall than a rifle
 * before it starts coming down — that falls out of per-weapon geometry for free.
 */
export const WALL_PRESS = {
  /** Start clearing this far before the barrel would actually touch, in metres. */
  margin: 0.3,
  /**
   * Fully cleared once the wall closes to this FRACTION of the muzzle's reach.
   *
   * Relative, not an absolute distance: measured reach runs from 0.48 m on the
   * Glock to 1.00 m on the SVD, so a fixed metre value would have saturated every
   * weapon at the same standoff and thrown away the whole point of the mechanic.
   * Worse, it sat behind the pistol's muzzle but half a metre in front of the
   * SVD's, so the two were inconsistent in opposite directions.
   */
  fullFrac: 0.55,
  /**
   * Above this press the trigger is dead. This is the actual fix for the bug —
   * a lowered muzzle sits inside the wall, so a round spawned there would appear
   * on the far side. Refusing the shot is also what the player expects: the gun
   * is visibly not pointed at anything.
   */
  blockFire: 0.7,
};
