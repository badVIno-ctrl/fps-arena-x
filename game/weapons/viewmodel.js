...
export class Viewmodel {
  ...
  update(dt, s) {
    ...
    // lowReady is GRADED, not a flag: mantling sends 1, but wall press sends a
    // 0..1 ramp so brushing a doorway barely moves the gun. A `? 1 : 0` here
    // would collapse a light 0.3 press into a full low-ready. Booleans still work
    // (preview.js passes false), since Number(false) === 0.
    const lowReadyTarget = clamp01(Number(s.lowReady) || 0);
    this.lowReadyT = damp(this.lowReadyT, lowReadyTarget, 8, dt);
    ...
    if (ads > 1e-4) {
      ...
      //
      // Prefer the MOUNTED OPTIC's optical axis. `rig.sightAxis()` has returned it
      // since it was written and nothing ever called it, so this solve always used
      // the weapon's own node: a red dot sits ~2cm above the irons and a scope
      // higher still, so every optic aimed at iron height - the glass was
      // decoration and the eye looked under it. Falls back to the authored node,
      // which is also what a build with no arsenal registered gets.
      if (this._sightSrc === undefined) this._sightSrc = this.ctx.peek?.('weapons') ?? null;
      const mounted = this._sightSrc?.sightPoint?.();
      if (mounted) this._sightLocal.set(mounted[0], mounted[1], mounted[2]);
      else this._sightLocal.copy(w.sight);
      this._sightLocal.applyQuaternion(this._adsQuat);
    ...
    /* -------- viewmodel FOV ------------------------------------------- */
    /**
     * Track the world camera instead of hardcoding 60.
     *
     * The viewmodel is drawn by its own camera so the gun can never clip into
     * geometry, but that camera still has to AGREE with the world one about how
     * wide a degree is. Pinned at 60 against a world at 80, every weapon was
     * magnified 1.33x — which is what made the AKM look oversized next to a
     * correctly-scaled city, even though its geometry and hip pose measured
     * right to the millimetre.
     *
     * It also silently broke whenever the world FOV moved: player/camera.js
     * drives it for sprint and ADS, so the gun's apparent size used to drift
     * against the world mid-sprint. Reading it here keeps the two locked at any
     * FOV, and `def.viewFov` still narrows the viewmodel on ADS to pull the
     * optic toward the eye.
     */
    const fovBase = this.ctx.camera?.fov ?? 60;
