 ...
 */
/** Digits 1-3 before the arsenal takes over. */
const BASE_SLOTS = ['rifle', 'smg', 'pistol'];
...
export class WeaponSystem {
  ...
  constructor() {
    ...
    this.states = new Map();
    this.activeId = 'rifle';
    /** Overwritten by the arsenal: which weapon each number key draws. */
    this.slotIds = null;
    this.debugMode = null;
    ...
    // Wall-press scratch. Preallocated: the probe runs every frame.
    this._pressDir = new THREE.Vector3();
    this._pressEye = new THREE.Vector3();
    this._pressMuzzle = new THREE.Vector3();
    this._wallPress = 0;
    // `loudness` multiplies the shot's audible range (1 = bare muzzle) and
    // `suppressed` picks the audio voice; both come from the mounted build.
    this._firePayload = {
      weapon: null,
    ...
    this._state = {
      ads: false,
      sprint: false,
      /** 0..1, not a flag — mantling sends 1, wall press ramps. */
      lowReady: 0,
      speed: 0,
      crouch: false,
      airborne: false,
  ...
  /**
   * How hard the muzzle is pressed into world geometry, 0..1.
   *
   * Measured along camera-forward from the eye, because that is the line the
   * bullet actually travels (see _fire: origin is the muzzle, direction is
   * camera-forward). The muzzle's own forward reach is derived from the live
   * viewmodel rather than hardcoded, so a pistol can be brought much closer to a
   * wall than a rifle before the gun starts coming down — which is the whole
   * point of the mechanic and falls out for free from per-weapon geometry.
   */
  _probeWallPress() {
    ...
    const cam = this.ctx.camera;
    if (!cam) return 0;
    // No drawn weapon means no barrel to bury; muzzleWorld would return the
    // origin and the reach below would be meaningless.
    if (!this.viewmodel?.active) return 0;

    cam.updateMatrixWorld();
    const dir = this._pressDir.set(0, 0, -1).applyQuaternion(cam.quaternion).normalize();
    const eye = this._pressEye.setFromMatrixPosition(cam.matrixWorld);

    // Forward reach of the muzzle, projected onto the aim line. Clamped low so a
    // clip that swings the muzzle behind the eye cannot invert the test.
    const muzzle = this.viewmodel.muzzleWorld(this._pressMuzzle);
    const reach = Math.max(0.12, muzzle.sub(eye).dot(dir));

    // Start clearing the weapon a little before contact, so the pose is already
    // moving by the time the barrel would intersect.
    const probe = reach + WALL_PRESS.margin;
    const hit = phys.raycast(eye, dir, probe, phys.MASK.WORLD);
    if (!hit?.hit) return 0;

    const dist = hit.distance ?? hit.t ?? probe;
    // Saturate at a FRACTION of this weapon's own reach, so a short pistol has to
    // get genuinely close before it comes down while a rifle clears early. An
    // absolute distance here would collapse all nine weapons onto one curve.
    const full = reach * WALL_PRESS.fullFrac;
    const t = (probe - dist) / Math.max(1e-4, probe - full);
    return clamp01(t);
  }
  ...
  update(dt, ctx) {
    ...
    // ---- wall press ------------------------------------------------------
    // Rounds leave the MUZZLE but fly along camera-forward, so pressing a long
    // barrel into a wall would otherwise spawn bullets on the far side of it and
    // leave the gun visibly buried in the geometry. Probe forward from the eye
    // and pull the weapon into a low-ready as the wall closes in, proportionally
    // rather than as a snap, so brushing a doorway barely moves the gun.
    this._wallPress = this._probeWallPress();
    // Mantling already forces a full low-ready; wall press grades into the same
    // pose, so take whichever is stronger and let one blend serve both.
    const mantling = player?.state === 'mantle' || player?.mantling === true;
    st.lowReady = mantling ? 1 : this._wallPress;
    ...
    if (live) {
      ...
      // `slotIds` is written by the arsenal subsystem; without it the three
      // built-in guns keep their historical digits.
      const slots = this.slotIds ?? BASE_SLOTS;
      if (input.pressed('Digit1') && slots[0]) this.setWeapon(slots[0]);
      if (input.pressed('Digit2') && slots[1]) this.setWeapon(slots[1]);
      if (input.pressed('Digit3') && slots[2]) this.setWeapon(slots[2]);
  ...
  lateUpdate(dt, ctx) {
    ...
    if (this._pendingShots > 0) {
      ...
      // Attachments change how loud a shot is, and loudness is what the AI hears
      // and what audio voices. `arsenal` injects `resolvedStats` when it registers
      // (it owns the attachment data; this engine must not import it), so an
      // unmodded install just reports a bare, full-volume shot. Resolved per shot
      // rather than per frame: firing is rare, and this cannot go stale.
      const rs = this.resolvedStats?.();
      this._firePayload.loudness = rs?.loudness ?? 1;
      this._firePayload.suppressed = rs?.silent ?? false;
