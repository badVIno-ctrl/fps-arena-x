...
export class AiSystem {
  ...
  _wireEvents(ctx) {
    ...
    on('weapon:fire', (e) => {
      ...
      // A gunshot is the loudest thing in the level, but HOW loud depends on the
      // muzzle device: a can is 0.35x, a brake is 1.15x (louder, that is the
      // trade). This used to be a flat 90m, which made the suppressor purely
  ...
  onAgentFire(agent, origin, dir) {
    ...
    // The player's zone colliders decide *where* a round lands, but hit-or-miss
    // stays with the cylinder test in _testPlayerHit — see the note there.
  ...
  _testPlayerHit(agent, origin, dir, end) {
    ...
    // Which zone did it hit? The cylinder test above stays the authority on
    // hit-or-miss because it *is* the bots' tuned accuracy model — swapping it
    // for an exact raycast would make every bot far deadlier and rebalance the
    // game by accident. But the player now has real head/torso/leg colliders,
    // so once the round is known to connect we can ask physics which one it
    // actually passed through. Before this, `headshot: false` was hardcoded and
    // the player was the only actor in the game who could not be headshot.
    let part = 'torso';
    const phys = this.ctx.peek('physics');
    if (phys?.raycast && phys.LAYER) {
      const zh = phys.raycast(origin, dir, maxT, phys.LAYER.PLAYER);
      if (zh?.hit && zh.part) part = zh.part;
    }
