...
export class NavGrid {
  ...
  /**
   * Drop walkable cells no bot can actually stand on.
   *
   * `build()` fires ONE ray per cell from above the entire map and takes the
   * first hit as the floor. Inside a building footprint that first hit is the
   * ROOF, so every roof samples as clean walkable ground while the interior
   * floors and stairwells underneath are never sampled at all. The grid is a
   * single-layer heightfield: it cannot represent a surface beneath another
   * surface, so no ground-to-roof route can exist at any maxStep.
   *
   * Measured on the shipped map: 798 regions, and only 76.8% of walkable cells
   * sit in the main one. The other 23% are rooftops and terraces, isolated by
   * the 3-13m drop around their perimeter. Raising maxStep does not help - at an
   * absurd 1.2m (bots levitating up waist-high walls) reachability moves 76.8%
   * to 77.5%, because those cells merge with each OTHER, never with the street.
   *
   * Leaving them flagged is actively harmful: `nearestWalkable` happily snaps to
   * them, so callers get a cell that looks fine and pathfinds to nothing. That
   * is exactly how the CTF base plinth froze a flag courier solid - the base
   * sampled walkable as a ten-cell island. So flood from the largest region and
   * clear the rest. Cheap, and it kills that whole class of silent freeze at the
   * source rather than one caller at a time.
   *
   * This does NOT make roofs unreachable for the player, who moves on physics
   * with step-up and is unaffected by the grid. It only stops the AI pretending
   * it can go where it cannot. Bots fighting on rooftops would need genuine
   * multi-layer nav plus stair links, which is a feature, not a fix.
   */
  pruneUnreachable() {
    ...
    const stack = [];
    let best = -1, bestN = 0, rid = 0;

    for (let s = 0; s < N; s++) {
      if (!this.flags[s] || region[s] !== -1) continue;
      stack.length = 0;
      stack.push(s);
      region[s] = rid;
      let n = 0;
      while (stack.length) {
        const cur = stack.pop();
        n++;
        const cx = cur % nx, cz = (cur / nx) | 0, cy = this.floor[cur];
        for (let d = 0; d < 8; d++) {
          const ix = cx + DX[d], iz = cz + DZ[d];
          if (!this.inside(ix, iz)) continue;
          const ni = this.index(ix, iz);
          if (!this.flags[ni] || region[ni] !== -1) continue;
          // Mirror findPath's own rules exactly, or the prune would keep cells
          // A* still refuses to enter: no corner cutting, and the same step cap.
          if (DX[d] && DZ[d]) {
            if (!this.flags[this.index(cx + DX[d], cz)] || !this.flags[this.index(cx, cz + DZ[d])]) continue;
          }
    ...
    if (best >= 0) {
      for (let i = 0; i < N; i++) {
        if (this.flags[i] && region[i] !== best) { this.flags[i] = 0; pruned++; }
      }
    }
    this.regionCount = rid;
    this.prunedCells = pruned;
    this.walkableCount -= pruned;
    return this;
  }
