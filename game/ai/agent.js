...
const STATE = {
  ...
  RETREAT: 'retreat',
  /**
   * Walking somewhere the game mode picked, for reasons the AI deliberately does
   * not understand (recovering the CTF flag today). Kept deliberately generic:
   * the agent has no business knowing what a flag is, so the mode supplies a
   * point and this state just walks to it. Combat still preempts it.
   */
  ERRAND: 'errand',
  DEAD: 'dead',
};
...
export class Agent {
  constructor(ai, opts = {}) {
    ...
    /** mode-assigned destination for STATE.ERRAND, or null. See assignErrand. */
    this.errand = null;
    this._errandPt = new THREE.Vector3();
    /** how far the approach ladder has backed off; see _errandDest */
    this._errandStep = 0;
    /** own scratch: _v2 is live inside _sense, so sharing it would alias */
    this._errandDestV = new THREE.Vector3();
  ...
  /**
   * Whether this agent is free to take a mode errand right now. Exposed as a
   * getter so callers never have to name AI states: `modes` talks to the engine
   * through ctx only and must not import STATE, and keeping the predicate next to
   * assignErrand is what stops the two from drifting apart.
   */
  get canErrand() {
    return this.alive && (
      this.state === STATE.IDLE
      ...
      || this.state === STATE.ERRAND
    );
  }

  /**
   * Send this agent to a point chosen by the game mode. Safe to call every frame:
   * it only re-enters ERRAND from the idle-ish states, so an ongoing fight, a
   * retreat or a suppression pin is never interrupted by an errand. That also
   * means a bot pulled into combat mid-errand resumes it once the fight ends,
   * without the mode having to track that.
   */
  assignErrand(point) {
    if (!this.alive || !point) return;
    // Only reset the approach ladder when the goal actually moves. The mode
    // re-asserts the same point every frame, and rebuilding the fallback each
    // time would restart the search forever and never let it settle.
    if (!this.errand || this._errandPt.distanceTo(point) > 1.0) {
      this._errandPt.copy(point);
      this._errandStep = 0;
    }
    this.errand = this._errandPt;
    if (this.state === STATE.IDLE || this.state === STATE.PATROL || this.state === STATE.ALERT) {
      this._setState(STATE.ERRAND);
    }
  }

  /**
   * Where to actually walk for the current errand.
   *
   * The exact goal is not always standable. The CTF flag's home base sits on a
   * plinth whose floor is 0.67m above the street, and the nav grid's maxStep is
   * 0.45m, so A* correctly reports the base cell as an island of ten cells that
   * no bot can climb - `_goTo` returns 0 and the courier stands still forever.
   * The player only reaches it because players move on physics with step-up,
   * not on the nav grid.
   *
   * So on failure, back off along the line from the goal towards us and try
   * again: the foot of the plinth is walkable and well inside the mode's
   * delivery radius. Handled here rather than in `modes` because the nav grid
   * and its step limit are the AI's business, and any future errand pointed at
   * a ledge, rooftop or podium hits exactly the same wall.
   */
  _errandDest(out) {
    const goal = this.errand;
    const back = this._errandStep * 1.6;
    if (back <= 0) return out.copy(goal);
    // Step back towards the agent, staying on the goal's own height so the
    // nearest-cell snap does not jump to a different storey.
    out.copy(goal);
    const dx = this.position.x - goal.x;
    const dz = this.position.z - goal.z;
    const len = Math.hypot(dx, dz);
    if (len < 1e-3) return out;
    const t = Math.min(back / len, 1);
    out.x += dx * t;
    out.z += dz * t;
    return out;
  }
  ...
  clearErrand() {
    this.errand = null;
    this._errandStep = 0;
    if (this.state === STATE.ERRAND) this._setState(this.patrolPoints ? STATE.PATROL : STATE.IDLE);
  }
  ...
  _think(dt) {
    ...
    switch (this.state) {
      ...
      case STATE.ERRAND: {
        ...
        // Same guard PATROL uses: a destination whose path is still queued is not
        // a destination reached, and re-requesting it every frame would keep the
        // A* budget permanently busy without ever moving.
        if (this.pathPending) break;
        // Re-path only when we have no path yet. The mode re-asserts the point
        // every frame, so anything less strict turns into an A* request per frame
        // and starves the shared budget.
        if (!this.hasMoveTarget) {
          const dest = this._errandDest(this._errandDestV);
          // 0 means "no route" (not "budget spent", which is -1 and sets
          // pathPending). Walk the approach ladder outwards; give up once even a
          // wide berth fails, so an impossible errand costs a bounded number of
          // solves instead of one per frame forever.
          if (this._goTo(dest) === false && !this.pathPending) {
            if (this._errandStep < 4) this._errandStep++;
            else this.clearErrand();
          }
        }
        break;
      }
