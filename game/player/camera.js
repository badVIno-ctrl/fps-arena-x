...
import { CAMERA, MOVE, HEALTH } from './tuning.js';
import {
...
export class CameraRig {
  ...
  update(dt, m, health) {
    ...
    // Resolved once, not every frame: the registry is stable after boot, but the
    // camera may well tick before weapons has registered.
    if (this._weapons === undefined) this._weapons = this.ctx.peek?.('weapons') ?? undefined;
    ...
    // Shock widens the sway on top of the above: taking a hard hit costs you the
    // follow-up shot, which is the mechanical point of it.
    amp *= lerp(1, HEALTH.shock.swayScale, clamp01(health.shock ?? 0));
    ...
    // How far ADS pulls the view in is a property of the SIGHT, not a global
    // constant. This was `cfg.adsFovScale` for everything, so a 4x PSO zoomed
    // exactly as much as iron sights - the entire point of mounting a scope did
    // nothing to the picture. It also silently ignored the per-weapon `adsFov`
    // already sitting in defs.js (0.74 rifle, 0.86 pistol).
    //
    // `zoom` resolves to the optic's FOV scale, or the weapon's own adsFov when
    // no optic is mounted; the global stays as the last-resort fallback so a
    // build with no arsenal registered still aims. Cheap per frame - stats() is
    // memoized on the rig.
    const zoom = this._weapons?.resolvedStats?.()?.zoom;
    this.fovAds = approach(this.fovAds, lerp(1, zoom ?? cfg.adsFovScale, ads), F.adsTau, dt);
