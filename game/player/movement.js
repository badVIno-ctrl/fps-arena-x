...
import { Stamina, STAMINA } from './load.js';
...
export class Movement {
  constructor(ctx, player) {
    ...
    this.sliding = false;
    /**
     * Stamina and carried load. Owned here rather than being its own system:
     * `_updateSprint` has to know whether a sprint is allowed inside the same
     * fixed step that sets `this.sprinting`, and an event round-trip would
     * deliver that answer a step late.
     */
    ...
    this.adsAmount = 0;
    /**
     * Extra speed multiplier for whatever the player is hauling, driven by the
     * game mode (the flag in CTF). Deliberately a neutral number rather than a
     * `carryingFlag` flag: movement has no business knowing what a flag is, and
     * this keeps mode rules out of the physics layer.
     */
    this.haulSpeedMul = 1;
  ...
  _updateSprint(cmd, rawInput, forwardIntent) {
    ...
    const wantSprint =
      ...
      !this.sliding &&
      this.adsAmount < 0.3 &&
      // Winded players walk. Sprint was previously free and unlimited, so the
      // pool is the only thing that ever says no on a legal input.
      this.stamina.canSprint() &&
      (c.grounded || this.sprinting);

    // Drive the pool from the state the last step settled on, then let the gate
    // above act on it next step. Draining here rather than in update() keeps the
    // cost tied to the fixed step, so it cannot vary with framerate.
    this.stamina.loadKg = this.player?.carriedKg ?? this.stamina.loadKg;
    this.stamina.update(this.ctx.time.fixed, {
      sprinting: this.sprinting,
      tacticalSprint: this.tacticalSprint,
      stance: this.stance,
      moving: rawInput > 0.05,
    });
  ...
  _beginSlide(cmd, wish, wishLen, currentSpeed) {
    ...
    this.stamina.spend(STAMINA.slideCost);
  }
  ...
  _doJump() {
    ...
    this.jumped = true;
    this.stamina.spend(STAMINA.jumpCost);
    this._setState('jump');
  }
  ...
  targetSpeed() {
    ...
    // Carried mass. Exactly 1.0 at the reference load the speeds in tuning.js
    // were balanced against, so this scales the roster apart without retuning
    // anything: an SVD is now measurably slower to carry than a Glock.
    base *= this.stamina.speedScale;
    // A winded player cannot hold top walking speed either, not just sprint.
    base *= lerp(1, 0.88, this.stamina.fatigue);
    // Whatever the mode has us hauling. Applied to the sprint branch too, so you
    // cannot outrun the penalty by holding shift with the flag.
    base *= this.haulSpeedMul;
    return base;
  }
