...
export class AudioSystem {
  ...
  _onFire(p) {
    ...
    // Suppression can arrive two ways: baked into the weapon (an integrally
    // suppressed gun, or the audio selftest) or as a per-shot flag from the
    // mounted muzzle device. The live game uses the latter, because the same
    // weapon can be fired with and without a can inside one match.
    if (p.suppressed || (w && typeof w === 'object' && w.suppressed)) {
      profile = WEAPON_PROFILES.suppressed;
    }
