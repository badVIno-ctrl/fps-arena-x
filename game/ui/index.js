 ...
 *                              stamina (0..1), exhausted, carriedKg }
...
export class UiSystem {
  ...
  async init(ctx) {
     ...
     *
     * Nothing was listening for this. Switching tab or minimising drops the
     * pointer lock, so the player came back to a match that had been running
     * unattended while they could neither see nor aim at it, and the lost-lock
     * watchdog only opened the menu once they were already dead. Freezing on
     * hide is also what stops the first frame back from consuming one enormous
     * dt (the engine clamps dt, but a clamped 0.1s step still teleports).
     */
    this._onVisibility = () => {
      if (document.visibilityState === 'hidden') this.menu.show();
    };
    ...
    on('damage:dealt', (e) => {
      ...
      if (e.killed) {
        ...
        this.killfeed.push({
          attacker: 'ВЫ',
          victim: e.target?.name ?? e.name ?? 'ВРАГ',
          headshot: !!e.headshot,
          mine: true,
        });
    ...
    on('actor:death', (e) => {
      ...
      this.killfeed.push({
        attacker: e?.by?.name ?? 'ВРАГ',
        victim: e?.actor?.name ?? 'БОЕЦ',
  ...
  /**
   * Stop treating "pointer not locked" as intent to pause until the lock is
   * actually held again. Called by PauseMenu.close(); see the note there.
   */
  disarmPointerLockWatchdog() {
    this._hadPointerLock = false;
  }
  ...
  lateUpdate(dt, ctx) {
    ...
    if (ps) {
      ...
      // The plate widget divides by maxArmour to size its three segments, so
      // without this it kept the 150 default and mis-scaled every plate.
      ...
      // `this.state` is an explicit whitelist, not a spread of the player state,
      // so a new field is invisible to the HUD until it is copied here.
      if (ps.stamina !== undefined) s.stamina = ps.stamina;
