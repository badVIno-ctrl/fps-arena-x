...
export class HealthFx {
  constructor(parent, chrome = parent) {
    ...
    // Bleed tag. Sits beside the Health label and only appears while a wound is
    // open, so the player can tell "regen is not starting" from "regen is broken".
    this.bleedTag = el('div', 'ow-bleed-tag', head, 'BLEEDING');
    ...
    // Stamina. Hidden at a full tank so the HUD stays quiet, and faded in as
    // soon as it is spent — running out has to be legible as a cost, or the
    // sudden drop to walking speed reads as the controls breaking.
    this.stam = el('div', 'ow-stam', this.vitals);
  ...
  update(dt, s) {
    ...
    // --- stamina ----------------------------------------------------------
    const stam = clamp01(s.stamina ?? 1);
    this.stamShown = damp(this.stamShown, stam, 14, dt);
    // Visible while spent, while recovering, and for a moment after topping up.
    const stamVis = stam < 0.995 ? 1 : 0;
    this.stamAlpha = damp(this.stamAlpha, stamVis, stamVis > 0 ? 18 : 3.5, dt);
    setStyle(this.stam, 'display', this.stamAlpha < 0.01 ? 'none' : '');
    if (this.stamAlpha >= 0.01) {
      setStyle(this.stam, 'opacity', this.stamAlpha.toFixed(3));
      setStyle(this.stamFill, 'transform', `scaleX(${this.stamShown.toFixed(4)})`);
      setClass(this.stam, 'spent', stam <= 0.3);
      setClass(this.stam, 'out', !!s.exhausted);
    }

    // --- bleeding ---------------------------------------------------------
    // Integrated from dt like everything else here, never a CSS keyframe, so the
    // pulse freezes with the game and the capture harness stays deterministic.
    const bleeding = !!s.bleeding;
    this.bleedAlpha = damp(this.bleedAlpha, bleeding ? 1 : 0, bleeding ? 16 : 6, dt);
    // 'block' explicitly, not '': the class carries display:none as its default,
    // so clearing the inline style would fall back to hidden and never show.
    setStyle(this.bleedTag, 'display', this.bleedAlpha < 0.01 ? 'none' : 'block');
    if (this.bleedAlpha >= 0.01) {
      this.bleedPhase = (this.bleedPhase + dt * 1.15) % 1;
      const pulse = 0.62 + 0.38 * (0.5 - 0.5 * Math.cos(this.bleedPhase * Math.PI * 2));
