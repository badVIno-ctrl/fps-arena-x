...
/**
 * Condensed stacks for the tactical register.
 *
 * These used to be system-only stacks ("Avenir Next Condensed", "DIN
 * Condensed"). That shipped a different interface depending on the machine:
 * both faces are macOS-only, so on Linux and Windows every fallback in the
 * chain was a NON-condensed face and the HUD lost the narrow look it was drawn
 * for. The condensed metrics are load-bearing here — the ammo readout and the
 * mode cards are laid out against them.
 *
 * The `--font-*` variables come from next/font in app/layout.tsx, so the faces
 * are self-hosted and identical everywhere. The old system names stay in the
 * chain: where they exist they are still a close match, and they cover the
 * first paint before the webfont lands.
 */
...
export const FONT_MONO =
  'var(--font-mono),"SF Mono",ui-monospace,"Roboto Mono",Menlo,monospace';
