...
/* stamina: a thin unlabelled rule under the health bar. Deliberately quieter
   than health and armour — it is a rhythm cue, not a number to read. */
.ow-stam { margin-top: calc(var(--u) * 1.1); }
.ow-stam-track {
  height: calc(3px * var(--k));
  background: rgba(5,9,12,.6);
  box-shadow: inset 0 0 0 1px rgba(0,0,0,.45), 0 0 0 1px rgba(216,232,240,.1);
}
.ow-stam-track > i {
  background: linear-gradient(to bottom, rgba(226,236,232,.85), rgba(150,166,160,.85));
  transition: none;
}
/* running low, then genuinely empty: the only time it raises its voice */
.ow-stam.spent .ow-stam-track > i {
  background: linear-gradient(to bottom, #ffd98a, #f2a01c);
}
.ow-stam.out .ow-stam-track > i {
  background: linear-gradient(to bottom, #ff8b7a, #e02414);
}

/* bleeding tag. Sits between the Health label and the numeral in the existing
   space-between head row, and is display:none while clotted so it cannot shift
   the numeral. Pulsed from dt in health.js, never a CSS animation, so it freezes
   with the game like every other value in this HUD. */
.ow-bleed-tag {
  display: none;
  font-size: calc(9px * var(--k)); letter-spacing:.2em; font-weight:700;
  color: #ff7a68;
  text-shadow: var(--o1), 0 0 calc(7px * var(--k)) rgba(224,36,20,.5);
  will-change: opacity;
}
