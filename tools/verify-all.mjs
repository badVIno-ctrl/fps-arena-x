...
check('the arsenal is registered, and only through its bridge', () => {
  // game/arsenal/index.js is the ONLY file down there allowed to declare an id:
  ...
  const claimed = [...SYSTEMS.values()].filter((f) => f.startsWith('game/arsenal/'));
  if (!has('game/arsenal/index.js')) return 'game/arsenal/index.js is missing: the kit is not wired into the match';
  if (claimed.length !== 1) return `expected exactly one arsenal subsystem file, found ${claimed.length || 'none'}`;
  if (claimed[0] !== 'game/arsenal/index.js') return `${claimed[0]} declares a subsystem id; only the bridge may`;
  const bridge = read('game/arsenal/index.js');
  ...
  return has('game/arsenal/defs.js') ? null : 'game/arsenal is missing entirely';
});
...
const DEV_ENTRIES = [
  'game/ai/preview.js',
  'game/audio/selftest.js',
  'game/fx/preview.js',
  'game/materials/preview.js',
  'game/physics/selftest.js',
  'game/weapons/preview.js',
];

function reachableFromMain() {
  const seen = new Set();
  const queue = ['game/boot.js'];
  while (queue.length) {
...
check('each new directory is covered by at least one gate', () => {
  ...
  const uncovered = ['arsenal', 'modes', 'net', 'shell'].filter(
    (dir) => !bodies.some((b) => b.includes(`game/${dir}/`) || b.includes(`'${dir}/`)),
  );
