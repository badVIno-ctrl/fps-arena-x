...
async function gunsmith() {
  ...
  check('opening the board stops the match and frees the mouse', () => {
    ...
    assert(open.includes("freeze('gunsmith')"), 'the match keeps running behind the board');
    ...
    assert(resume.includes("thaw('gunsmith')"), 'time scale is not restored');
    assert(resume.includes('requestPointerLock'), 'the mouse is not recaptured');
    return 'pause and resume are symmetric';
  });

  /**
   * The board and the pause menu both freeze the match clock and can overlap.
   * When each one saved and restored `time.scale` itself, the second to open
   * captured the already-zeroed value and the last to close restored 0 — the
   * match froze with no menu on screen and no way back. freeze/thaw are
   * reference counted by owner, so the order stops mattering; this guards
   * against anyone reintroducing a private save/restore of the shared scale.
   */
  check('nothing outside the clock assigns time.scale directly', () => {
    const offenders = [];
    for (const [name, src] of Object.entries({ shell, 'ui/menu.js': read('ui/menu.js') })) {
      // `scale: 1` in the clock literal and reads like `time.scale` are fine;
      // only assignments to a .scale that belongs to a time object are not.
      if (/\btime\s*\.\s*scale\s*=/.test(src) || /\b_prev(Time)?Scale\b/.test(src)) {
        offenders.push(name);
      }
    }
    assert(
      offenders.length === 0,
      `${offenders.join(' and ')} still save/restore time.scale by hand instead of freeze()/thaw()`
    );
    return 'the clock owns its own scale';
  });
