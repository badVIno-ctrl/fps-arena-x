...
async function syntax() {
  group('syntax');
  /**
   * `node --check` parses the file as a real ES module, which is the same parser
   * that will load it at runtime.
   *
   * The previous approach masked import/export lines with regex and fed the
   * remainder to vm.compileFunction. That produced 14 false failures: the regex
   * `^\s*import\s[^;]*;?$` is single-line, so every multi-line import block was
   * left half-deleted, and the resulting fragment could not parse. Files that
   * `node --check` accepts without complaint were being reported as broken —
   * worse than no gate, because it trains you to ignore the output.
   */
  ...
  for (const file of ALL) {
    ...
    check(`parses ${rel}`, () => {
      assert(!err, err ? String(err.stderr || err.message).split('\n').slice(0, 4).join(' ') : '');
    });
  }
...
async function contract() {
  ...
  /**
   * Isolation: never import another subsystem's `index.js`.
   *
   * The thing worth forbidding is reaching for another subsystem's SYSTEM — its
   * class, its mutable per-match state, its lifecycle. That is what `index.js`
   * exports, and grabbing it directly bypasses the registry's init ordering and
   * creates exactly the kind of cycle that made `arsenal` call into an
   * uninitialised `shell` earlier in this build. Runtime state goes through
   * ctx.get() / ctx.peek() so the registry stays in charge.
   *
   * Importing a subsystem's LEAF modules is a different act and is allowed.
   * `weapons/geometry.js` and `weapons/parts.js` are a stateless parts kit —
   * pure functions in, geometry out — and the arsenal exists precisely so those
   * 300 lines of solved millimetres are written once instead of nine times.
   * `ui/util.js` is likewise just constants and math.
   *
   * The old rule banned any cross-directory import at all, which flagged all
   * six of those legitimate library imports. A gate that fires on correct code
   * is worse than no gate: it gets ignored, and then it hides the real thing.
   */
  for (const file of NEW) {
    const code = readFileSync(file, 'utf8');
    const rel = relative(ROOT, file);
    ...
    for (const m of code.matchAll(/from\s+['"](\.\.[^'"]+)['"]/g)) {
      ...
      if (owner === mine || owner === '' || owner === 'core') continue;
      const isSystemEntry = /(^|\/)index\.js$/.test(target);
      check(`${rel} does not import ${owner}/index.js`, () => {
        assert(
          !isSystemEntry,
          `imports the ${owner} system directly: use ctx.get('${owner}') so the ` +
            `registry controls init order`,
        );
      });
    }
  }
